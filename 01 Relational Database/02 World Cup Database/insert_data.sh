...
do
  ...
  TEAM_ID_OPO=$($PSQL "SELECT team_id FROM teams WHERE name='$OPPONENT'")

  if [[ -n $TEAM_ID_WIN || $TEAM_ID_OPO ]]
  then
    if [[ $YEAR!="year" ]]
    then
      INSERT_GAMES=$($PSQL "INSERT INTO games(year, round, winner_id, opponent_id, winner_goals, opponent_goals) VALUES('$YEAR', '$ROUND', '$TEAM_ID_WIN', '$TEAM_ID_OPO', '$WINNER_GOALS', '$OPPONENT_GOALS')")
